...
void PlayerPhysics::integrate(GameObject& target, float dt, bool enabled) {
	...
	for (int i = 0; i < 8 && remaining_dt > 0.0f; ++i) {
		...
		for (Block* block : blocks) {
			...
			bool underfoot_level = block->position.y + 0.5f <= target.position.y - target.size.y * 0.5f + 0.01f;
			if (underfoot_level) {
				bool directly_below = abs(block->position.x - target.position.x) < (target.size.x * 0.5f + 0.5f)
					&& abs(block->position.z - target.position.z) < (target.size.z * 0.5f + 0.5f);
...
bool PlayerPhysics::is_position_clear(const GameObject& probe) {
	...
	for (Block* block : gather_candidate_blocks(bounds)) {
		GameObject block_box;
		block_box.position = block->position;
		block_box.size = vec3(1.0f);
